...
function computeYTD(
  ...
  const ytd_actual = sales
    .filter((s) => { const d = new Date(s.fecha); return d >= startActual && d <= fechaReferencia })
    .reduce((a, s) => a + s.unidades, 0)

  const ytd_anterior = sales
    .filter((s) => { const d = new Date(s.fecha); return d >= startAnterior && d <= endAnterior })
    .reduce((a, s) => a + s.unidades, 0)
...
function computeClientesDormidos(
  ...
  const today = sales.length > 0
    ...
    : new Date()
  const byCliente: Record<string, SaleRecord[]> = {}
  sales.forEach((s) => {
    if (!s.cliente) return
    if (!byCliente[s.cliente]) byCliente[s.cliente] = []
    byCliente[s.cliente].push(s)
  })

  const dormidos: ClienteDormido[] = []
  for (const [cliente, records] of Object.entries(byCliente)) {
    const sorted = [...records].sort((a, b) => new Date(b.fecha).getTime() - new Date(a.fecha).getTime())
    const ultima_compra = new Date(sorted[0].fecha)
    const dias_sin_actividad = Math.floor(
      (today.getTime() - ultima_compra.getTime()) / 86400000
    )
    ...
    const valor_historico = records.reduce(
      (a, s) => a + (s.venta_neta ?? s.unidades),
      0
    )
    ...
    dormidos.push({
      ...
      dias_sin_actividad,
      valor_historico,
      compras_historicas: records.length,
    })
  }

  return dormidos.sort((a, b) => b.valor_historico - a.valor_historico)
}
...
export function computeCommercialAnalysis(
  ...
  // fechaReferencia = max date in sales (for YTD and "dormidos")
  const fechaReferencia = sales.length > 0
    ? sales.reduce((max, s) => { const d = new Date(s.fecha); return d > max ? d : max }, new Date(0))
    : today

  // Pre-group by vendedor (single pass over all sales) for performance
  ...
  const byVendedor = new Map<string, SaleRecord[]>()
  sales.forEach((s) => {
    if (!s.vendedor) return
    let arr = byVendedor.get(s.vendedor)
    if (!arr) { arr = []; byVendedor.set(s.vendedor, arr) }
    arr.push(s)
  })

  const vendedores = Array.from(byVendedor.keys())
  const vendorAnalysis = vendedores.map((v) =>
    analyzeVendor(
      v, byVendedor.get(v)!, metas, selectedPeriod,
      diasTranscurridos, diasTotales, diasRestantes, fechaReferencia,
      periodStart, periodEnd, prevStart, prevEnd,
    )
  )

  // Team stats (reuse pre-computed boundaries)
  const periodSales = sales.filter((s) => s.fecha >= periodStart && s.fecha <= periodEnd)
  const prevSales = sales.filter((s) => s.fecha >= prevStart && s.fecha <= prevEnd)
  ...
  const hasCliente = sales.some((s) => s.cliente != null)
  const clientesDormidos = hasCliente
    ? computeClientesDormidos(sales, config.dias_dormido_threshold)
    : []

  const concentracionRiesgo = hasCliente
    ? computeConcentracion(sales, year, month)
    : []

  // Cuántos clientes suman > threshold%
  let acum = 0
  ...
  const hasProducto = sales.some((s) => s.producto != null)
  ...
  if (hasProducto && inventory.length > 0) {
    ...
    const recentProducts = new Set(
      sales.filter((s) => s.fecha >= last15 && s.producto).map((s) => s.producto)
    )
...
export function getMejoresPeriodosVendedor(
  ...
  for (let i = 1; i <= n; i++) {
    ...
    let m = month - i
    while (m < 0) { y--; m += 12 }
    const total = salesInPeriod(sales, y, m)
      .filter((s) => s.vendedor === vendedor)
      .reduce((a, s) => a + s.unidades, 0)
    results.push(total)
  }
...
export function computeCategoriasInventario(
  ...
  // Calcular PM3: promedio de ventas de los 3 meses cerrados anteriores al período seleccionado
  ...
  sales.forEach((s) => {
    ...
    if (!ultimoMovMap[s.producto] || d > ultimoMovMap[s.producto]) {
      ultimoMovMap[s.producto] = d
    }
  })
