...
import { TrendingUp, TrendingDown, Minus, Calendar, ChevronRight, ChevronDown, ArrowUp, ArrowDown, Settings2 } from 'lucide-react'
...
export default function RendimientoPage() {
  ...
  const { sales, metas, dataAvailability, selectedPeriod, configuracion } = useAppStore()
  ...
  // Filtered sales (for chart + pivot)
  const filteredSales = useMemo(() =>
    selectedVendor === 'todos' ? sales : sales.filter((s) => s.vendedor === selectedVendor),
  [sales, selectedVendor])

  // ── Chart data ────────────────────────────────────────────────────────────
  const chartData = useMemo(() => {
    return MESES.map((label, monthIdx) => {
      const currSales = salesInPeriod(filteredSales, currentYear, monthIdx)
      const currVal = useVentaNeta ? currSales.reduce((a, s) => a + (s.venta_neta ?? 0), 0) : currSales.reduce((a, s) => a + s.unidades, 0)

      const prevSales = salesInPeriod(filteredSales, prevYear, monthIdx)
      ...
      if (showForecast && isCurrentYear && monthIdx > currentMonth) {
        const ytdCurr = Array.from({ length: currentMonth + 1 }, (_, i) => {
          const s = salesInPeriod(filteredSales, currentYear, i)
          return useVentaNeta ? s.reduce((a, v) => a + (v.venta_neta ?? 0), 0) : s.reduce((a, v) => a + v.unidades, 0)
        }).reduce((a, b) => a + b, 0)
        const ytdPrev = Array.from({ length: currentMonth + 1 }, (_, i) => {
          const s = salesInPeriod(filteredSales, prevYear, i)
          return useVentaNeta ? s.reduce((a, v) => a + (v.venta_neta ?? 0), 0) : s.reduce((a, v) => a + v.unidades, 0)
        }).reduce((a, b) => a + b, 0)
        const gf = ytdPrev > 0 ? ytdCurr / ytdPrev : 1
        forecast = prevVal > 0 ? Math.round(prevVal * gf) : null
      }
  ...
  }, [filteredSales, metas, currentYear, prevYear, currentMonth, useVentaNeta, showForecast, showBudget, isCurrentYear, dataAvailability.has_metas])
  ...
  const ytdStats = useMemo(() => {
    ...
    for (let m = 0; m <= currentMonth; m++) {
      const cs = salesInPeriod(filteredSales, currentYear, m)
      const ps = salesInPeriod(filteredSales, prevYear, m)
  ...
  }, [filteredSales, currentYear, prevYear, currentMonth, useVentaNeta, isCurrentYear])

  // ── Pivot computation ─────────────────────────────────────────────────────
  const pivotTree = useMemo(() => {
    const currSales = filteredSales.filter((s) => new Date(s.fecha).getFullYear() === currentYear)
    const prevSales = filteredSales.filter((s) => new Date(s.fecha).getFullYear() === prevYear)
    return buildPivotTree(currSales, prevSales, metas, pivotDims, currentYear, 'root', 0, null, null)
  }, [filteredSales, metas, pivotDims, currentYear, prevYear])
    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-20 animate-in fade-in duration-700">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-zinc-500 mt-1">Comparativa año actual vs año anterior</p>
        </div>
        <div className="flex items-center gap-2">
          <Calendar className="w-4 h-4 text-zinc-600" />
          <span className="text-sm font-medium text-zinc-400">{currentYear} vs {prevYear}</span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {isCurrentYear && (
            <button onClick={() => setShowForecast(!showForecast)} className={cn('px-3 py-2 rounded-lg text-xs font-bold border transition-all', showForecast ? 'bg-[#00B894]/10 border-[#00B894]/30 text-[#00B894]' : 'bg-zinc-900 border-zinc-800 text-zinc-500')}>Proyección</button>
          )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4">
          <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600 mb-1">YTD {currentYear}</p>
        ...
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4">
          <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600 mb-1">vs {prevYear}</p>
        ...
        {isCurrentYear && (
          <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4">
            ...
            <p className="text-[10px] text-zinc-600 mt-1">Cierre estimado {currentYear}</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6">
        ...
        <ResponsiveContainer width="100%" height={360}>
          <LineChart data={chartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
            ...
            <Line type="monotone" dataKey="anterior" name={hasPrevYearData ? String(prevYear) : `${prevYear} — sin datos`} stroke={hasPrevYearData ? '#52525b' : '#27272a'} strokeWidth={hasPrevYearData ? 2 : 1} strokeDasharray={hasPrevYearData ? undefined : '3 3'} opacity={hasPrevYearData ? 1 : 0.3} dot={false} connectNulls />
            <Line type="monotone" dataKey="actual" name={String(currentYear)} stroke="#00B894" strokeWidth={3} dot={(props: any) => { const { cx, cy, payload } = props; if (!payload.isCurrent) return <g key={`dot-${cx}`} />; return <circle key={`dot-${cx}`} cx={cx} cy={cy} r={5} fill="#00B894" stroke="#000" strokeWidth={2} /> }} connectNulls />
